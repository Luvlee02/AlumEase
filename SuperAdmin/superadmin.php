...
<div class="flex-container">
  <div class="card  text-white" style="width:400px; ; height: 150px; background-color: #47C1BF;">
    ...
    <i class="bi bi-search"></i>
     ...
     <center> <h5> 
      <?php 
            $hired = "SELECT * FROM `application` WHERE status IN ('6')";
            $query = mysqli_query($conn,$hired);

            if($hired_alumni = mysqli_num_rows($query)){
              echo "$hired_alumni";
            }else{
              echo "No Data";
            }
  ...
  <div class="card  text-white" style="width:400px; height: 150px; background-color: #5e308c;">
    ...
    <i class="bi bi-mortarboard"></i>
        ...
        <center> <h5> 
          <?php 
            $alumni = "SELECT * FROM `users` WHERE `user_type` IN ('5') AND `user_status` IN ('1')";
            $total_alumni_query = mysqli_query($conn,$alumni);

            if($alumni_total = mysqli_num_rows($total_alumni_query)){
              echo "$alumni_total";
            }else{
              echo "No Data";
            }
